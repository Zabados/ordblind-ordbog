---

## HEADWORD [REQUIRED]

```
headword: diskutere
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: verb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  infinitive: diskutere
  present: diskuterer
  past: diskuterede
  past_participle: diskuteret
  present_participle: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: DISKUTERE
ipa: TODO
syllables: dis-ku-te-re
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: to discuss
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Lad os ikke diskutere det.
    english: Let's not discuss it.
    source: tatoeba
    source_id: 9690298
  - danish: Nogle mennesker hader at diskutere.
    english: Some people hate to argue.
    source: tatoeba
    source_id: 8047637
  - danish: Nogle mennesker elsker at diskutere.
    english: Some people love to argue.
    source: tatoeba
    source_id: 8047634
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 3132
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-diskutere-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
