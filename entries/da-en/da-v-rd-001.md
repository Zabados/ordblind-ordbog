---

## HEADWORD [REQUIRED]

```
headword: værd
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections: SKIP
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: VEHRD
ipa: /ˈvɛːˀr/
syllables: værd
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: worth
secondary_translations:
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Er det det værd?
    english: Is it worth it?
    source: tatoeba
    source_id: 8206396
  - danish: Det er umagen værd.
    english: It is worthwhile.
    source: tatoeba
    source_id: 4255216
  - danish: Det er forsøget værd.
    english: It's worth a try.
    source: tatoeba
    source_id: 5343960
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2190
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-v-rd-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
