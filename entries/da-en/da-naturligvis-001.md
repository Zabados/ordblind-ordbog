---

## HEADWORD [REQUIRED]

```
headword: naturligvis
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adverb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections: SKIP
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: NATURLIGVIS
ipa: /natuːrliviːs/
syllables: na-tur-lig-vis
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: of course
secondary_translations:
  - naturally
  - obviously
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Naturligvis.
    english: Naturally.
    source: tatoeba
    source_id: 8285222
  - danish: Du har naturligvis ret.
    english: You're right, of course.
    source: tatoeba
    source_id: 8657475
  - danish: Naturligvis har han ret.
    english: Of course, he is right.
    source: tatoeba
    source_id: 2282004
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - education
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1174
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-naturligvis-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
