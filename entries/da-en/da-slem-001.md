---

## HEADWORD [REQUIRED]

```
headword: slem
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  positive: slem
  neuter: slemt
  plural_definite: slemme
  comparative: TODO
  superlative: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: SLEM
ipa: [ˈslɛmˀ]
syllables: slem
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: bad, ill-behaved, mean
secondary_translations:
  - serious in a bad way, grievous
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Du har været en slem hund.
    english: You've been a bad dog.
    source: tatoeba
    source_id: 10774629
  - danish: En slem forkølelse har afholdt mig fra at studere i denne uge.
    english: A bad cold has kept me from studying this week.
    source: tatoeba
    source_id: 12009316
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - health
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 11534
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-slem-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
