---

## HEADWORD [REQUIRED]

```
headword: million
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: million
  definite_singular: millionen
  indefinite_plural: millioner
  definite_plural: millionerne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: MILLION
ipa: [miliˈoˀn]
syllables: mil-li-on
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: A , 106.
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: En million er et syvcifret tal.
    english: One million is a seven digit number.
    source: tatoeba
    source_id: 4463404
  - danish: Ordbogen indeholder cirka en halv million ord.
    english: The dictionary contains about half a million words.
    source: tatoeba
    source_id: 618764
  - danish: Ordbogen indeholder omtrent en halv million ord.
    english: The dictionary contains about half a million words.
    source: tatoeba
    source_id: 621707
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1526
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-million-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
