---

## HEADWORD [REQUIRED]

```
headword: behov
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: behov
  definite_singular: behovet
  indefinite_plural: SKIP
  definite_plural: behovene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: BEHOW
ipa: /behɔv/
syllables: be-hov
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: need, requirement
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Folk har en tendens til at undervurdere deres fremtidige behov.
    english: People have a tendency to underestimate their future needs.
    source: tatoeba
    source_id: 2260483
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 664
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-behov-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
