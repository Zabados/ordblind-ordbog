---

## HEADWORD [REQUIRED]

```
headword: modsige
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: verb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  infinitive: modsige
  present: modsiger
  past: modsagde
  past_participle: modsagt
  present_participle: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: MODSIYE
ipa: TODO
syllables: mod-si-ge
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: to contradict
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Du skal ikke modsige mig!
    english: Don't contradict me.
    source: tatoeba
    source_id: 11985726
  - danish: Lad være med at modsige mig!
    english: Don't contradict me.
    source: tatoeba
    source_id: 11985725
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 82987
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-modsige-001
created: 2026-05-18
last_modified: 2026-05-18
source_wiktionary: false
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
