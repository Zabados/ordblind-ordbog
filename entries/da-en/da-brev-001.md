---

## HEADWORD [REQUIRED]

```
headword: brev
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: brev
  definite_singular: brevet
  indefinite_plural: breve
  definite_plural: brevene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: BREW
ipa: /breːˀv/
syllables: brev
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: letter
secondary_translations:
  - note
  - epistle
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Her er dit brev.
    english: Here is your letter.
    source: tatoeba
    source_id: 7094942
  - danish: Jeg skrev et brev.
    english: I wrote a letter.
    source: tatoeba
    source_id: 1528911
  - danish: Han skrev et brev.
    english: He wrote a letter.
    source: tatoeba
    source_id: 4375572
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2010
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-brev-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
