---

## HEADWORD [REQUIRED]

```
headword: lægge
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: verb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  infinitive: lægge
  present: lægger
  past: lagde
  past_participle: lagt
  present_participle: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: LEHGYE
ipa: [ˈlɛɡ̊ə]
syllables: læg-ge
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: to lay
secondary_translations:
  - to put, place
  - to lay (an egg)
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Du skal ikke lægge på.
    english: Don't you hang up on me.
    source: tatoeba
    source_id: 12493464
  - danish: Lad os lægge kortene på bordet.
    english: Let's put our cards on the table.
    source: tatoeba
    source_id: 7810527
  - danish: Tom vil ikke lægge mærke til det.
    english: Tom won't notice.
    source: tatoeba
    source_id: 10953895
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1024
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-l-gge-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
