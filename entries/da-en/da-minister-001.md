---

## HEADWORD [REQUIRED]

```
headword: minister
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: minister
  definite_singular: ministeren
  indefinite_plural: ministre
  definite_plural: ministrene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: MINISTER
ipa: TODO
syllables: mi-ni-ster
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: minister; secretary of state
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Vores minister er vegetar og ateist.
    english: Our minister is a vegetarian and an atheist.
    source: tatoeba
    source_id: 2474942
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2303
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-minister-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
