---

## HEADWORD [REQUIRED]

```
headword: ende
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: ende
  definite_singular: enden
  indefinite_plural: ender
  definite_plural: enderne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: ENDE
ipa: /ɛnə/
syllables: en-de
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: end
secondary_translations:
  - point, prong, tine
  - behind, bottom, buttocks, backside, bum, fanny
  - to end, finish
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Hun vandt i sidste ende.
    english: She came out on top.
    source: tatoeba
    source_id: 2475165
  - danish: Vi er nået til vejs ende.
    english: We reached the end of the road.
    source: tatoeba
    source_id: 10620319
  - danish: Tom er nået til vejs ende.
    english: Tom reached the end of the road.
    source: tatoeba
    source_id: 10620329
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 791
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-ende-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
