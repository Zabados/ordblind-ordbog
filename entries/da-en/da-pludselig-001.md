---

## HEADWORD [REQUIRED]

```
headword: pludselig
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  positive: pludselig
  neuter: pludseligt
  plural_definite: pludselige
  comparative: TODO
  superlative: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: PLUDSELEE
ipa: TODO
syllables: plud-se-lig
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: sudden
secondary_translations:
  - overnight
  - suddenly, all of a sudden
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Han kom pludselig.
    english: He came suddenly.
    source: tatoeba
    source_id: 8180259
  - danish: Tom frøs pludselig.
    english: Tom suddenly felt cold.
    source: tatoeba
    source_id: 11767338
  - danish: Vores tog stoppede pludselig.
    english: Our train stopped suddenly.
    source: tatoeba
    source_id: 12963597
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 945
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-pludselig-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
