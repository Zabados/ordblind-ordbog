---

## HEADWORD [REQUIRED]

```
headword: vilkår
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: vilkår
  definite_singular: vilkåret
  indefinite_plural: SKIP
  definite_plural: vilkårene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: VILKAWR
ipa: TODO
syllables: vil-kår
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: term, condition, stipulation, circumstance
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Ikke på vilkår!
    english: No way!
    source: tatoeba
    source_id: 2342498
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2669
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-vilk-r-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
