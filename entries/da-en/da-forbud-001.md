---

## HEADWORD [REQUIRED]

```
headword: forbud
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: forbud
  definite_singular: forbuddet eller (uofficielt) -et
  indefinite_plural: SKIP
  definite_plural: forbuddene eller (uofficielt) -ene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: FORBUD
ipa: [ˈfɒːˌb̥uð]
syllables: for-bud
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: ban
secondary_translations:
  - prohibition
  - harbinger
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Rygning forbudt.
    english: No smoking.
    source: tatoeba
    source_id: 2054031
  - danish: Strengt forbudt.
    english: Strictly forbidden.
    source: tatoeba
    source_id: 12296932
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1610
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-forbud-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: false
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
