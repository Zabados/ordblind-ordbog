---

## HEADWORD [REQUIRED]

```
headword: reducere
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: verb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  infinitive: reducere
  present: reducerer
  past: reducerede
  past_participle: reduceret
  present_participle: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: RETHUCERE
ipa: TODO
syllables: re-du-ce-re
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: to reduce, lessen
secondary_translations:
  - to reduce the score and thereby closer to the opponent
  - make (something) more concentrated by letting it boil so that some of the liquid evaporates
  - to reduce fractions
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Kan du reducere prisen en smule?
    english: Could you reduce the price a little?
    source: tatoeba
    source_id: 4371542
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2501
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-reducere-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
