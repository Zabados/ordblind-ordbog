---

## HEADWORD [REQUIRED]

```
headword: opmærksom
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  positive: opmærksom
  neuter: opmærksomt
  plural_definite: opmærksomme
  comparative: TODO
  superlative: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: OPMEHRKSOM
ipa: /ˈɔpˌmæɐ̯ɡsɒm/
syllables: op-mærk-som
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: alert, attentive, mindful, observant, observing, watchful
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Vær opmærksom!
    english: Pay attention!
    source: tatoeba
    source_id: 1851815
  - danish: Når man oversætter, skal man være opmærksom på »falske venner«!
    english: When translating, one should be aware of "false friends"!
    source: tatoeba
    source_id: 10499278


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1216
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-opm-rksom-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
