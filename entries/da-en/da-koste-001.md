---

## HEADWORD [REQUIRED]

```
headword: koste
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: verb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  infinitive: koste
  present: koster
  past: kostede
  past_participle: kostet
  present_participle: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: KOSTE
ipa: TODO
syllables: ko-ste
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: to cost; to have a price; to be for sale
secondary_translations:
  - to boss around
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Det vil koste 30 euro.
    english: That will cost thirty euros.
    source: tatoeba
    source_id: 1854579
  - danish: Det vil koste tredive euro.
    english: That will cost thirty euros.
    source: tatoeba
    source_id: 1854580



---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - money
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1600
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-koste-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
