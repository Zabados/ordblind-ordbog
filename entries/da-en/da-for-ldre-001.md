---

## HEADWORD [REQUIRED]

```
headword: forældre
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: SKIP
  definite_singular: SKIP
  indefinite_plural: forældre
  definite_plural: forældrene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: TODO
ipa: TODO
syllables: TODO
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: parents
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Af den grund bør unge og deres forældre læse den.
    english: SKIP
    source: leipzig
    source_id: SKIP
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 228401
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-for-001
created: 2026-05-19
last_modified: 2026-05-19
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Auto-generated from gap analysis. Needs manual review.
```
