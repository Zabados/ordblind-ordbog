---

## HEADWORD [REQUIRED]

```
headword: hår
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: hår
  definite_singular: håret
  indefinite_plural: SKIP
  definite_plural: hårene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: HAWR
ipa: /hɔːˀr/
syllables: hår
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: hair
secondary_translations:
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Red dit hår.
    english: Comb your hair.
    source: tatoeba
    source_id: 10666095
  - danish: Toms hår er vådt.
    english: Tom's hair is wet.
    source: tatoeba
    source_id: 3674138
  - danish: Tom har gråt hår.
    english: Tom has gray hair.
    source: tatoeba
    source_id: 5786074
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 3077
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-h-r-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
