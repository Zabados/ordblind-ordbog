---

## HEADWORD [REQUIRED]

```
headword: sol
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: sol
  definite_singular: solen
  indefinite_plural: sole
  definite_plural: solene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: SOL
ipa: /soːl/
syllables: sol
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: sun
secondary_translations:
  - solution
  - note
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Katten slikkede sol.
    english: The cat sunned itself.
    source: tatoeba
    source_id: 7089286
  - danish: Det franske ord »soleil« betyder »sol«.
    english: The French word "soleil" means "sun".
    source: tatoeba
    source_id: 12406702
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - weather
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2561
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-sol-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
