---

## HEADWORD [REQUIRED]

```
headword: italiensk
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  positive: italiensk
  neuter: SKIP
  plural_definite: italienske
  comparative: TODO
  superlative: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: ITALIENSK
ipa: [itˢalˈjɛˀnsɡ̊]
syllables: ita-li-ensk
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: Italian
secondary_translations:
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Hun er italiensk.
    english: She is Italian.
    source: tatoeba
    source_id: 4064550
  - danish: Taler du italiensk?
    english: Do you speak Italian?
    source: tatoeba
    source_id: 615148
  - danish: Han taler italiensk.
    english: He speaks Italian.
    source: tatoeba
    source_id: 12961420
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 8285
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-italiensk-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
