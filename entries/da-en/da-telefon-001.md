---

## HEADWORD [REQUIRED]

```
headword: telefon
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: telefon
  definite_singular: telefonen
  indefinite_plural: telefoner
  definite_plural: telefonerne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: TELEFON
ipa: /teləfoːn/
syllables: te-le-fon
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: telephone
secondary_translations:
  - phone number, telephone number , abbreviated tlf.
  - call, phone call, telephone call
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Har I en telefon?
    english: Do you have a phone?
    source: tatoeba
    source_id: 9771084
  - danish: Oplad din telefon.
    english: Charge your phone.
    source: tatoeba
    source_id: 10712371
  - danish: Har du en telefon?
    english: Do you have a phone?
    source: tatoeba
    source_id: 9771076
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - technology
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2000
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-telefon-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
