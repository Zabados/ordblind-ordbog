---

## HEADWORD [REQUIRED]

```
headword: sten
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: sten
  definite_singular: betydning 1 og 4-6: -en
  indefinite_plural: SKIP
  definite_plural: stenene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: STEN
ipa: /steːn/
syllables: sten
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: stone
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Gem dig bag den sten.
    english: Hide behind that rock.
    source: tatoeba
    source_id: 12387354
  - danish: Det var de vises sten.
    english: This was the Philosopher's Stone.
    source: tatoeba
    source_id: 4823537
  - danish: Den var hård som sten.
    english: It was as hard as rock.
    source: tatoeba
    source_id: 670059
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2474
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-sten-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
