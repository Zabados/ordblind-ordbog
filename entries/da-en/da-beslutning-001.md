---

## HEADWORD [REQUIRED]

```
headword: beslutning
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: beslutning
  definite_singular: beslutningen
  indefinite_plural: beslutninger
  definite_plural: beslutningerne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: BESLUTNING
ipa: TODO
syllables: be-slut-ning
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: decision
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Det var en dum beslutning.
    english: That was a dumb decision.
    source: tatoeba
    source_id: 7786461
  - danish: Denne beslutning er endelig.
    english: This decision is final.
    source: tatoeba
    source_id: 7272762


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 818
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-beslutning-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
