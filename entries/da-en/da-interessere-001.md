---

## HEADWORD [REQUIRED]

```
headword: interessere
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: verb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  infinitive: interessere
  present: interesserer
  past: interesserede
  past_participle: interesseret
  present_participle: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: INTERESSERE
ipa: [entˢʁ̥ɐˈseˀɐ]
syllables: in-ter-es-se-re
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: to hold someone's interest
secondary_translations:
  - be interested
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Hvornår begyndte du at interessere dig for at lære sprog?
    english: How did you become interested in studying languages?
    source: tatoeba
    source_id: 1934375
  - danish: Siden hvornår er du begyndt at interessere dig for politik?
    english: Since when do you care about politics?
    source: tatoeba
    source_id: 4493223
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 15535
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-interessere-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
