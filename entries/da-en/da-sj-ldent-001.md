---

## HEADWORD [REQUIRED]

```
headword: sjælden
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  positive: sjælden
  neuter: sjældent
  plural_definite: sjældne
  comparative: TODO
  superlative: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: SYEHLDENT
ipa: TODO
syllables: sjæl-dent
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: rare; uncommon; infrequent
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Tom ler sjældent.
    english: Tom seldom laughs.
    source: tatoeba
    source_id: 10572311
  - danish: Det sker sjældent.
    english: That rarely happens.
    source: tatoeba
    source_id: 5194100
  - danish: Jeg synger sjældent.
    english: I rarely sing.
    source: tatoeba
    source_id: 12430168
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2259
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-sj-ldent-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: false
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
