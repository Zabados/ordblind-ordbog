---

## HEADWORD [REQUIRED]

```
headword: alkohol
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: alkohol
  definite_singular: alkoholen
  indefinite_plural: alkoholer
  definite_plural: alkoholerne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: ALKOHOL
ipa: TODO
syllables: al-ko-hol
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: alcohol
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Jeg drikker alkohol.
    english: I drink alcohol.
    source: tatoeba
    source_id: 6311085
  - danish: Alkohol løser ingen problemer.
    english: Alcohol doesn't solve any problems.
    source: tatoeba
    source_id: 2603473
  - danish: Min mor drikker aldrig alkohol.
    english: My mum never drinks alcohol.
    source: tatoeba
    source_id: 13024852
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2653
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-alkohol-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
