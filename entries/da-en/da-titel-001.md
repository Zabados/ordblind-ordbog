---

## HEADWORD [REQUIRED]

```
headword: titel
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: titel
  definite_singular: titlen
  indefinite_plural: titler
  definite_plural: titlerne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: TITEL
ipa: TODO
syllables: ti-tel
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: A title.
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Tom er i gang med at skrive en bog, men den har endnu ikke en titel.
    english: Tom is writing a book, but it does not have a title yet.
    source: tatoeba
    source_id: 6173077
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 5327
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-titel-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
