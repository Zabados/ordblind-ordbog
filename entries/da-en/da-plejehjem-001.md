---

## HEADWORD [REQUIRED]

```
headword: plejehjem
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: plejehjem
  definite_singular: plejehjemmet
  indefinite_plural: SKIP
  definite_plural: plejehjemmene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: PLAYEHYEM
ipa: TODO
syllables: ple-je-hjem
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: nursing home; care home
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Min mor har arbejdet på et plejehjem.
    english: My mom worked in a retirement home.
    source: tatoeba
    source_id: 11670020
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1114
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-plejehjem-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: false
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
