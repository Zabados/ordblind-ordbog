---

## HEADWORD [REQUIRED]

```
headword: fælles
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  positive: fælles
  neuter: SKIP
  plural_definite: SKIP
  comparative: TODO
  superlative: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: FEHLLES
ipa: [ˈfɛlˀəs]
syllables: fæl-les
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: common, communal
secondary_translations:
  - shared, jointly owned
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Vi har et fælles mål.
    english: We have a common goal.
    source: tatoeba
    source_id: 11937428
  - danish: I to har en masse til fælles.
    english: You two have a lot in common.
    source: tatoeba
    source_id: 5321725
  - danish: Vi har ikke meget til fælles.
    english: We don't have much in common.
    source: tatoeba
    source_id: 9817410
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 754
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-f-lles-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
