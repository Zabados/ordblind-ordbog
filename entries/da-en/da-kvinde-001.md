---

## HEADWORD [REQUIRED]

```
headword: kvinde
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: kvinde
  definite_singular: kvinden
  indefinite_plural: kvinder
  definite_plural: kvinderne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: KVINDE
ipa: /ˈkvenə/
syllables: kvin-de
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: A woman.
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Jeg er kvinde.
    english: I'm a woman.
    source: tatoeba
    source_id: 7756525
  - danish: Hun er kvinde.
    english: She's a woman.
    source: tatoeba
    source_id: 12572663


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 556
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-kvinde-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
