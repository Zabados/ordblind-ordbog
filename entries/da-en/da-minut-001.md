---

## HEADWORD [REQUIRED]

```
headword: minut
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: minut
  definite_singular: minuttet
  indefinite_plural: minutter
  definite_plural: minutterne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: MINUT
ipa: TODO
syllables: mi-nut
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: minute
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Kun et minut.
    english: Just a minute.
    source: tatoeba
    source_id: 663417
  - danish: Hvert minut tæller.
    english: Every minute counts.
    source: tatoeba
    source_id: 3674171
  - danish: Ét minut er tres sekunder.
    english: One minute has sixty seconds.
    source: tatoeba
    source_id: 12030390
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - time
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2630
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-minut-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
