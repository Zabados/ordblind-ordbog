---

## HEADWORD [REQUIRED]

```
headword: ordentligt
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections: SKIP
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: ORDENTLIGT
ipa: TODO
syllables: or-dent-ligt
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: accurately, awfully, carefully, correctly, decently, methodically, neatly, properly, regularly, steadily, thoroughly, tidily, well
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Spørg ordentligt!
    english: Ask properly.
    source: tatoeba
    source_id: 12311693
  - danish: Opfør dig ordentligt.
    english: Behave yourself.
    source: tatoeba
    source_id: 7789934
  - danish: Opfør jer ordentligt.
    english: Behave yourself.
    source: tatoeba
    source_id: 7789935
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - food
  - drink
  - transport
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2697
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-ordentligt-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
