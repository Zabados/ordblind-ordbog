---

## HEADWORD [REQUIRED]

```
headword: måde
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: måde
  definite_singular: måden
  indefinite_plural: måder
  definite_plural: måderne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: TODO
ipa: [ˈmɔːðə]
syllables: TODO
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: method, way
secondary_translations:
  - moderation
  - to measure
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: ADD’en er på ingen måde nem at få øje på.
    english: SKIP
    source: leipzig
    source_id: SKIP
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 60674
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-m-001
created: 2026-05-19
last_modified: 2026-05-19
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Auto-generated from gap analysis. Needs manual review.
```
