---

## HEADWORD [REQUIRED]

```
headword: brand
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: brand
  definite_singular: branden
  indefinite_plural: brande
  definite_plural: brandene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: BRAND
ipa: /branˀ/
syllables: brand
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: fire
secondary_translations:
  - smut
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: "Brand" råbte han.
    english: "Fire!" he cried.
    source: tatoeba
    source_id: 618787
  - danish: Tom døde i en brand.
    english: Tom died in a fire.
    source: tatoeba
    source_id: 3674123
  - danish: Ved brand alarmér 119.
    english: In case of fire, call 119.
    source: tatoeba
    source_id: 7826779
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1079
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-brand-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
