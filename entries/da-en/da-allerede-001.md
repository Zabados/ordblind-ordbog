---

## HEADWORD [REQUIRED]

```
headword: allerede
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adverb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections: SKIP
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: ALLERETHE
ipa: TODO
syllables: al-le-re-de
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: already (prior to some time, so soon)
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Det regner allerede.
    english: It rains already.
    source: tatoeba
    source_id: 13117510
  - danish: Jeg er allerede rig.
    english: I'm already rich.
    source: tatoeba
    source_id: 1999437
  - danish: Jeg ved det allerede.
    english: I already know that.
    source: tatoeba
    source_id: 5947927
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - time
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 299
frequency_tier: core
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-allerede-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
