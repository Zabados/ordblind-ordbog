---

## HEADWORD [REQUIRED]

```
headword: dets
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: pronoun
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections: SKIP
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: DETS
ipa: TODO
syllables: dets
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: its, possessive form of
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Dets navn er Tom.
    english: Its name is Tom.
    source: tatoeba
    source_id: 7101545
  - danish: Man kan kende et menneske på dets venner.
    english: You can know a man by his friends.
    source: tatoeba
    source_id: 2220184
  - danish: Bedøm ikke et menneske ud fra dets udseende.
    english: Don't judge a person by his appearance.
    source: tatoeba
    source_id: 13214766
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - bureaucracy
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 3609
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-dets-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
