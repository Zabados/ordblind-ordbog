---

## HEADWORD [REQUIRED]

```
headword: lov
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: lov
  definite_singular: loven
  indefinite_plural: love
  definite_plural: lovene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: LOW
ipa: /ˈlɔv/
syllables: lov
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: law
secondary_translations:
  - permission
  - praise
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Det er en dum lov.
    english: It's a stupid law.
    source: tatoeba
    source_id: 6482812
  - danish: Det er en tosset lov.
    english: It's a stupid law.
    source: tatoeba
    source_id: 6482810
  - danish: Kender du Murphys lov?
    english: Do you know Murphy's law?
    source: tatoeba
    source_id: 8331879
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - politics
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 507
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-lov-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
