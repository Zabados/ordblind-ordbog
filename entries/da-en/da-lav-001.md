---

## HEADWORD [REQUIRED]

```
headword: lav
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  positive: lav
  neuter: lavt
  plural_definite: lave
  comparative: TODO
  superlative: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: LAW
ipa: /ˈlaːˀv/
syllables: lav
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: low
secondary_translations:
  - short; shallow
  - humble; inferior; base
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Min puls er lav.
    english: My pulse is low.
    source: tatoeba
    source_id: 4479142
  - danish: Priserne er lave denne uge.
    english: The prices are low this week.
    source: manual
    source_id: SKIP
  - danish: Temperaturen er lav om natten.
    english: The temperature is low at night.
    source: manual
    source_id: SKIP
```



---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 2891
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-lav-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
