---

## HEADWORD [REQUIRED]

```
headword: eftermiddag
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: eftermiddag
  definite_singular: eftermiddagen
  indefinite_plural: eftermiddage
  definite_plural: eftermiddagene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: EFTERMIDDAG
ipa: TODO
syllables: ef-ter-mid-dag
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: afternoon (part of the day between noon and evening)
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: God eftermiddag!
    english: Good afternoon!
    source: tatoeba
    source_id: 4395129
  - danish: Vi ses i eftermiddag.
    english: I'll see you this afternoon.
    source: tatoeba
    source_id: 6902658


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - time
  - culture
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 538
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-eftermiddag-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
