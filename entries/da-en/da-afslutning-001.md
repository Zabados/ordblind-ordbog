---

## HEADWORD [REQUIRED]

```
headword: afslutning
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: afslutning
  definite_singular: afslutningen
  indefinite_plural: afslutninger
  definite_plural: afslutningerne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: AFSLUTNING
ipa: TODO
syllables: af-slut-ning
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: end, finale
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Handelshindringer blev ophævet efter krigens afslutning.
    english: Trade barriers were lifted after the war ended.
    source: tatoeba
    source_id: 3856281
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 3110
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-afslutning-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
