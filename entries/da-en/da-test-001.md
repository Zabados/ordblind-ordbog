---

## HEADWORD [REQUIRED]

```
headword: test
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: test
  definite_singular: testen
  indefinite_plural: test eller -s eller (uofficielt) -er
  definite_plural: testene eller (uofficielt) -erne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: TEST
ipa: /ˈtɛsd/
syllables: test
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: A , assessment or examination.
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Dna-test viste at han var uskyldig.
    english: DNA tests showed he was innocent.
    source: tatoeba
    source_id: 1921124
  - danish: DNA-test viste at han var uskyldig.
    english: DNA tests showed he was innocent.
    source: tatoeba
    source_id: 2513042
  - danish: Har du nogensinde taget en IQ-test?
    english: Have you ever taken an IQ test?
    source: tatoeba
    source_id: 10244872
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - education
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 820
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-test-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
