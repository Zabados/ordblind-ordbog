---

## HEADWORD [REQUIRED]

```
headword: sko
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: sko
  definite_singular: skoen
  indefinite_plural: SKIP
  definite_plural: skoene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: SKO
ipa: /sɡoːˀ/
syllables: sko
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: shoe
secondary_translations:
  - to shoe
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: I sælger sko.
    english: You sell shoes.
    source: tatoeba
    source_id: 4172783
  - danish: Du sælger sko.
    english: You sell shoes.
    source: tatoeba
    source_id: 4172782
  - danish: Vi sælger sko.
    english: We sell shoes.
    source: tatoeba
    source_id: 3248679
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 4122
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-sko-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: Phonetic approximation needs manual review.
```
