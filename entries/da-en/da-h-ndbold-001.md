---

## HEADWORD [REQUIRED]

```
headword: håndbold
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: håndbold
  definite_singular: håndbolden
  indefinite_plural: håndbolde
  definite_plural: håndboldene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: HAWNDBOLD
ipa: TODO
syllables: hånd-bold
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: handball (sport)
secondary_translations:
  - handball (ball)
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Håndbold er en meget populær sport i Danmark.
    english: Handball is a very popular sport in Denmark.
    source: tatoeba
    source_id: 2373025
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - body
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1837
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-h-ndbold-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
