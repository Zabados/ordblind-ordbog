---

## HEADWORD [REQUIRED]

```
headword: digital
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adjective
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections:
  positive: digital
  neuter: digitalt
  plural_definite: digitale
  comparative: TODO
  superlative: TODO
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: DIYITAL
ipa: /diɡiˈtæˀl/
syllables: di-gi-tal
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: digital representing discrete values
secondary_translations:
  - digital of or relating to computers or the Information Age
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Udviklingen indenfor digital teknologi skrider frem i et halsbrækkende tempo.
    english: Digital technology is advancing at a breakneck pace.
    source: tatoeba
    source_id: 7786428
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 3635
frequency_tier: general
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: false
```

---

## PIPELINE METADATA

```
entry_id: da-digital-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
