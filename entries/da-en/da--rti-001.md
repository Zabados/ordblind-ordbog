---

## HEADWORD [REQUIRED]

```
headword: årti
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: årti
  definite_singular: årtiet
  indefinite_plural: årtier
  definite_plural: årtierne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: AWRTI
ipa: /ɔrtiː/
syllables: år-ti
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: decade (a period of ten years)
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Et årti er et tidsrum på ti år.
    english: A decade is a span of ten years.
    source: tatoeba
    source_id: 8805594
  - danish: Et årti er en periode på ti år.
    english: A decade is a span of ten years.
    source: tatoeba
    source_id: 8805595
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - time
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 6371
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da--rti-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
