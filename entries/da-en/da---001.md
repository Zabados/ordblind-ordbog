---

## HEADWORD [REQUIRED]

```
headword: ø
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: ø
  definite_singular: ø'et
  indefinite_plural: ø'er
  definite_plural: ø'erne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: UR
ipa: [ˈøˀ]
syllables: ø
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: island
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Vi bor på en ø.
    english: We live on an island.
    source: tatoeba
    source_id: 11176718
  - danish: Jeg ejer den ø.
    english: I own that island.
    source: tatoeba
    source_id: 10706276
  - danish: Cypern er en ø.
    english: Cyprus is an island.
    source: tatoeba
    source_id: 10706269
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 5679
frequency_tier: rare
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da---001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
