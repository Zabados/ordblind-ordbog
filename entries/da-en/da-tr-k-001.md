---

## HEADWORD [REQUIRED]

```
headword: træk
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: træk
  definite_singular: betydning 1-9: -ket
  indefinite_plural: SKIP
  definite_plural: trækkene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: TREHK
ipa: /trɛk/
syllables: træk
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: pull
secondary_translations:
  - touch, feature, trait
  - move
  - migration, flight
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Træk dit sværd.
    english: Draw your sword.
    source: tatoeba
    source_id: 7017195
  - danish: Træk en streg her.
    english: Draw a line here.
    source: tatoeba
    source_id: 10758902
  - danish: Layla havde så smukke træk.
    english: Layla had such beautiful features.
    source: tatoeba
    source_id: 10663849
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - TODO
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1195
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-tr-k-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
