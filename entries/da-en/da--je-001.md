---

## HEADWORD [REQUIRED]

```
headword: øje
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: et
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: øje
  definite_singular: betydning 1 og 2: -t
  indefinite_plural: øjne
  definite_plural: øjnene
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: URYE
ipa: /ɔjə/
syllables: øje
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: eye
secondary_translations:
  - to see, watch
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Hold øje med ham.
    english: Keep an eye on him.
    source: tatoeba
    source_id: 9836513
  - danish: Hold øje med Tom.
    english: Keep your eye on Tom.
    source: tatoeba
    source_id: 11813592



---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - body
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1109
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da--je-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
