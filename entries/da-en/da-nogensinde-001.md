---

## HEADWORD [REQUIRED]

```
headword: nogensinde
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: adverb
gender: SKIP
```

### Inflections [OPTIONAL]

```
inflections: SKIP
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: NOYENSINDE
ipa: /ˈnoːɡənˈsenə/
syllables: no-gen-sin-de
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: ever, at some time
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Har du nogensinde røget?
    english: Have you ever smoked?
    source: tatoeba
    source_id: 2355617
  - danish: Har du nogensinde snorket?
    english: Have you ever snored?
    source: tatoeba
    source_id: 4928653
  - danish: Går Tom nogensinde med hat?
    english: Does Tom ever wear a hat?
    source: tatoeba
    source_id: 8825890
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - time
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 1307
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-nogensinde-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
