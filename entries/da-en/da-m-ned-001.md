---

## HEADWORD [REQUIRED]

```
headword: måned
direction: DA→EN
```

---

## GRAMMAR [REQUIRED]

```
pos: noun
gender: en
```

### Inflections [OPTIONAL]

```
inflections:
  indefinite_singular: måned
  definite_singular: måneden
  indefinite_plural: måneder
  definite_plural: månederne
```

---

## PRONUNCIATION [REQUIRED]

```
phonetic_plain: MAWNED
ipa: /ˈmɔːnəð/
syllables: må-ned
stoed: TODO
```

---

## TRANSLATION [REQUIRED]

```
primary_translation: month
secondary_translations:
  - SKIP
```

---

## REGISTER [OPTIONAL]

```
register: TODO
domain: TODO
formality: TODO
```

---

## EXAMPLE SENTENCES [REQUIRED]

```yaml
examples:
  - danish: Hvad måned er vi i?
    english: What is the current month?
    source: tatoeba
    source_id: 9859399
  - danish: Marts måned er slut.
    english: March is over.
    source: tatoeba
    source_id: 13756302
  - danish: Marts måned er omme.
    english: March is over.
    source: tatoeba
    source_id: 13756275
```


---

## RELATED WORDS [OPTIONAL]

```
related: TODO
```

---

## THEMATIC TAGS [REQUIRED]

```
tags:
  - time
```

---

## FREQUENCY [REQUIRED]

```
frequency_rank: 603
frequency_tier: common
```

---

## LAYOUT HINTS [MANUAL]

```
layout:
  print_emphasis: normal
  flag_false_friend: false
  flag_false_friend_note: SKIP
  flag_spelling_trap: false
  flag_pronunciation_trap: true
```

---

## PIPELINE METADATA

```
entry_id: da-m-ned-001
created: 2026-05-05
last_modified: 2026-05-05
source_wiktionary: true
source_tatoeba: true
source_leipzig: true
review_status: draft
reviewed_by: SKIP
notes: SKIP
```
